...
//type *params.CliqueConfig struct {
...
type Historysnap struct {
	...
	SignersHash map[string]struct{} `json:"signers"` // 当前的授权用户
...
// 为创世块使用
func newHistorysnap(config *params.CliqueConfig, sigcache *lru.ARCCache, number uint64, hash common.Hash, signers []common.Address) *Historysnap {
	snap := &Historysnap{
		config:   config,
		sigcache: sigcache,
		Number:   number,
...
// 判断投票的有效性
func (s *Historysnap) validVoteHash(hash string, authorizeHash bool) bool {
	_, signerHash := s.SignersHash[hash]
	//如果已经在，应该删除，如果不在申请添加才合法
...
// 判断当前的次序
/*
func (s *Historysnap) inturnHash(number uint64, signerHash string) bool {
	signers, offset := s.signers(), 0
	for offset < len(signers) && signers[offset] != signer {
		offset++
	}
...
func (s *Historysnap) apply(headers []*types.Header) (*Historysnap, error) {
	...
	for _, header := range headers {
		...
		// signer 是否在Signers中，如果不在则返回错误
		if _, ok := snap.Signers[signer]; !ok {
			return nil, errUnauthorized
		}
		
		// signer 是否在 recent中，说明已经签过名
		// 防止连续放入
